...
| Encryption | AES-256-GCM | Cloud backup encryption |
| Export Formats | JSON, CSV, encrypted JSON | QR code only |
| Biometric Lock | Yes | Yes |
| Screen Capture Block | Yes | No |
| Cloud Backup | Manual (encrypted) | Google account sync |
| Custom Icons | Yes | No |
...
Enable PIN/biometric unlock AND a separate Aegis master password for defense-in-depth.
...
Enable device lock and avoid relying solely on Google Authenticator for critical accounts. Use hardware security keys (YubiKey) as the primary second factor.
